 ...
 *
 * Usage:
 *   node fetch-turkey-facilities.js
 *
...
async function fetchQuery({ label, query }, retries = 4) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    const res = await fetch(
      ...
      { headers: { 'User-Agent': 'turkey-facilities-fetcher/1.0 (github.com/m-yasutake)' } }
    );
    if (res.ok) {
...
async function main() {
  ...
  const outPath = path.join(__dirname, '..', 'assets', 'turkey-facilities.json');
