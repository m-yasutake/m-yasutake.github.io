...
// Check admin status via Firestore REST API (single HTTP request — avoids loading
// the Firestore SDK and its persistent WebChannel on every page load).
let _adminStatusCache = null; // { email, isAdmin } — per page-load cache
async function _checkAdminViaRest(user) {
  if (_adminStatusCache && _adminStatusCache.email === user.email) {
    return _adminStatusCache.isAdmin;
  }
  ...
  if (!resp.ok) {
    _adminStatusCache = { email: user.email, isAdmin: false };
    return false;
  }
  const data = await resp.json();
  const vals = (data.fields && data.fields.emails &&
                data.fields.emails.arrayValue &&
                data.fields.emails.arrayValue.values) || [];
  const isAdmin = vals.some(v => v.stringValue === user.email);
  _adminStatusCache = { email: user.email, isAdmin };
  return isAdmin;
}
...
async function _onAuthStateChanged(user) {
  ...
  // Show Admin nav link only when signed in as an admin
  document.querySelectorAll('.admin-only-link').forEach(el => { el.style.display = 'none'; });
  const adminGate = document.getElementById('adminGate');
  const adminDashboard = document.getElementById('adminDashboard');
  if (user) {
    try {
      const isAdmin = await _checkAdminViaRest(user);
      document.querySelectorAll('.admin-only-link').forEach(el => { el.style.display = isAdmin ? '' : 'none'; });
      if (adminGate && adminDashboard) {
        if (isAdmin) {
          ...
          if (gateBtn) { gateBtn.disabled = false; gateBtn.textContent = 'Sign In with Google'; }
        }
      }
    } catch (e) {
      // Admin check failed — leave admin UI hidden
      if (adminGate && adminDashboard) {
        adminGate.style.display = '';
        adminDashboard.style.display = 'none';
      }
